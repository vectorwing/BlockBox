...
public class PlaqueRenderer implements BlockEntityRenderer<PlaqueBlockEntity>
{
	...
	public void render(PlaqueBlockEntity blockEntity, float partialTick, PoseStack poseStack, MultiBufferSource bufferSource, int packedLight, int packedOverlay) {
		...
		PlaqueBlock block = (PlaqueBlock) state.getBlock();

		this.translateSign(poseStack, -block.getYRotationDegrees(state), state);
		this.renderSignText(block, state, blockEntity.getLevel(), blockEntity.getFrontText(), poseStack, bufferSource, packedLight, blockEntity.getTextLineHeight(), blockEntity.getMaxTextLineWidth());
	}
	...
	private void renderSignText(PlaqueBlock block, BlockState state, Level level, SignText text, PoseStack poseStack, MultiBufferSource buffer, int packedLight, int lineHeight, int maxWidth) {
		...
		this.translateSignText(poseStack, TEXT_OFFSET);
		int lineHeightOffset = 4 * lineHeight / 2;
		FormattedCharSequence[] renderMessages = text.getRenderMessages(Minecraft.getInstance().isTextFilteringEnabled(), component -> {
		...
		for (int lineIndex = 0; lineIndex < 4; lineIndex++) {
			FormattedCharSequence message = renderMessages[lineIndex];
			float x = (float) (-this.font.width(message) / 2);

			Matrix4f textOffset = new Matrix4f(poseStack.last().pose());
			this.font.drawInBatch(message, x, (float) (lineIndex * lineHeight - lineHeightOffset), textColor, false, textOffset, buffer, Font.DisplayMode.POLYGON_OFFSET, 0, packedLight);

			textOffset.translate(1.0F, 1.0F, -0.03F);
			this.font.drawInBatch(message, x, (float) (lineIndex * lineHeight - lineHeightOffset), highlightColor, false, textOffset, buffer, Font.DisplayMode.POLYGON_OFFSET, 0, packedLight);
		}
