...
public class ItemModels extends ItemModelProvider
{
	...
	public ResourceLocation resourceBlock(String path) {
		return ResourceLocation.fromNamespaceAndPath(BlockBox.MODID, "block/" + path);
	}

	public ResourceLocation resourceItem(String path) {
		return ResourceLocation.fromNamespaceAndPath(BlockBox.MODID, "item/" + path);
	}
	...
	public static <T> Collection<T> takeAll(Set<? extends T> src, T... items) {
		...
		for (T item : items) {
			if (!src.contains(item)) {
				BlockBox.LOGGER.warn("Item {} not found in set", item);
			}
		}
		if (!src.removeAll(ret)) {
			BlockBox.LOGGER.warn("takeAll array didn't yield anything ({})", Arrays.toString(items));
		}
		return ret;
	}
